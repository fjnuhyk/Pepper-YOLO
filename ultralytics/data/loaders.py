...
class LoadStreams:
    ...
    def __init__(self, sources="file.streams", vid_stride=1, buffer=False):
        ...
        for i, s in enumerate(sources):
            ...
            LOGGER.info(f"{st}Success 鉁?({self.frames[i]} frames of shape {w}x{h} at {self.fps[i]:.2f} FPS)")
            ...
            if len(self.imgs[i]) < 30:
                ...
                if n % self.vid_stride == 0:
                    ...
                    if not success:
                        ...
                        LOGGER.warning("WARNING 鈿狅笍 Video stream unresponsive, please check your IP camera connection.")
                        cap.open(stream)
                    if self.buffer:
                        self.imgs[i].append(im)
                    else:
    ...
    def close(self):
        ...
        for cap in self.caps:
            try:
                ...
                LOGGER.warning(f"WARNING 鈿狅笍 Could not release VideoCapture object: {e}")
    ...
    def __next__(self):
        ...
        for i, x in enumerate(self.imgs):
            ...
            while not x:
                ...
                x = self.imgs[i]
                if not x:
                    LOGGER.warning(f"WARNING 鈿狅笍 Waiting for stream {i}")
...
class LoadTensor:
    ...
    def _single_check(im, stride=32):
        ...
        s = (
            f"WARNING 鈿狅笍 torch.Tensor inputs should be BCHW i.e. shape(1, 3, 640, 640) "
        ...
        if im.max() > 1.0 + torch.finfo(im.dtype).eps:
            LOGGER.warning(
                f"WARNING 鈿狅笍 torch.Tensor inputs should be normalized 0.0-1.0 but max value is {im.max()}. "
