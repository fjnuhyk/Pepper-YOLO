...
class BaseModel(nn.Module):
    ...
    def _profile_one_layer(self, m, x, dt):
        ...
        if type(x) is list:
            ...
            bs = x.size(0)
        o = thop.profile(m, inputs=[x.copy() if c else x], verbose=False)[0] / 1E9 * 2 / bs if thop else 0
        t = time_sync()
        for _ in range(10):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):
        ...
        if self.yaml["backbone"][0][2] == "Silence":
            LOGGER.warning(
                "WARNING 鈿狅笍 YOLOv9 `Silence` module is deprecated in favor of nn.Identity. "
        ...
        if nc and nc != self.yaml["nc"]:
            ...
            self.yaml["nc"] = nc
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=ch, verbose=verbose, warehouse_manager=self.warehouse_manager)
        ...
        if isinstance(m, (DETECT_CLASS + SEGMENT_CLASS + POSE_CLASS + OBB_CLASS + V10_DETECT_CLASS)):
            ...
            try:
                m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(2, ch, s, s))])
                ...
                if 'Not implemented on the CPU' in str(e) or 'Input type (torch.FloatTensor) and weight type (torch.cuda.FloatTensor)' in str(e) or 'CUDA tensor' in str(e) or 'is_cuda()' in str(e):
                    ...
                    m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(2, ch, s, s).to(torch.device('cuda')))])
                else:
                    raise e
    ...
    def _predict_augment(self, x):
        ...
        if getattr(self, "end2end", False):
            LOGGER.warning(
                "WARNING 鈿狅笍 End2End model does not support 'augment=True' prediction. "
...
def temporary_modules(modules=None, attributes=None):
    ...
    Example:
        ...
        with temporary_modules({'old.module': 'new.module'}, {'old.module.attribute': 'new.module.attribute'}):
            import old.module
            from old.module import attribute
        ```
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):
        ...
        if m in {
            ...
            C2f_FMB, LDConv, C2f_gConv, RepNCSPELAN4AKConv1
        }:
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
